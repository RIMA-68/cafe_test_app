...
st.markdown(
    ...
    <style>
      ...
      div.stButton > button{
        width:100%; font-family:'Galmuri11',monospace; font-size:16px; color:var(--black);
        background:var(--ink); border:none; border-radius:8px; padding:17px 18px;
        box-shadow:0 5px 0 var(--pink-deep), 0 6px 0 var(--black);
        transition:transform .06s, box-shadow .06s; line-height:1.5; margin-bottom:8px;
      }
      ...
      .hud-bar{ display:flex; justify-content:space-between; align-items:center;
        font-family:'Galmuri9',monospace; font-size:12px; color:var(--pink-soft); letter-spacing:.5px; margin-bottom:20px;}
      ...
      .q-title{ text-align:center; color:var(--ink); font-family:'Galmuri11',monospace;
        font-size:clamp(20px,5.4vw,25px); line-height:1.6; margin:0 auto 28px; max-width:430px;
        text-shadow:0 2px 0 var(--pink-deep);}
...
def question(step, title, opt_a, opt_b, key, val_a, val_b, nxt):
    hud(step, 3)
    st.markdown("<div class='q-eyebrow'>☕ CAFE TYPE TEST</div>", unsafe_allow_html=True)
    st.markdown(f"<h2 class='q-title'>{title}</h2>", unsafe_allow_html=True)
    if st.button(opt_a, key=f"{key}_a"):
        st.session_state.ans[key] = val_a; go(nxt)
    if st.button(opt_b, key=f"{key}_b"):
...
if page == "intro":
    ...
    # (폴백) 임베드 버튼이 막힌 환경이면 주석 해제
    # if st.button("▶ 시작하기"): go("q1")

elif page == "q1":
    question(
        1, "쉬는 시간, 얼마나 멀리까지 갈 수 있어?",
