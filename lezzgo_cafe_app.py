...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  .pin{ position:relative; width:26px; height:36px; }
  .pin svg{ display:block; filter:drop-shadow(0 3px 4px rgba(0,0,0,.5)); }
...
<script>
  ...
  function pinIcon(fill, star){
    var inner = star
      ? "<path d='M13 7 l1.6 3.4 3.7 .4 -2.8 2.5 .8 3.6 -3.3 -1.9 -3.3 1.9 .8 -3.6 -2.8 -2.5 3.7 -.4 z' fill='#170d1d'/>"
      : "<circle cx='13' cy='13' r='4.6' fill='#fff'/>";
    var svg = "<div class='pin'><svg width='26' height='36' viewBox='0 0 26 36'>"
      + "<path d='M13 0 C5.8 0 0 5.8 0 13 C0 22 13 36 13 36 C13 36 26 22 26 13 C26 5.8 20.2 0 13 0 Z' fill='"+fill+"' stroke='#fff' stroke-width='2'/>"
      + inner + "</svg></div>";
    return L.divIcon({className:'', html:svg, iconSize:[26,36], iconAnchor:[13,36], popupAnchor:[0,-32]});
  }
  function acadIcon(){
    var svg = "<div class='pin' style='width:32px;height:44px'><svg width='32' height='44' viewBox='0 0 32 44'>"
      + "<path d='M16 0 C7.2 0 0 7.2 0 16 C0 27 16 44 16 44 C16 44 32 27 32 16 C32 7.2 24.8 0 16 0 Z' fill='#ffce5c' stroke='#1a1320' stroke-width='2.4'/>"
      + "<path d='M16 8 l2 4.1 4.5 .5 -3.4 3 1 4.4 -4.1 -2.3 -4.1 2.3 1 -4.4 -3.4 -3 4.5 -.5 z' fill='#1a1320'/>"
      + "</svg></div>";
    return L.divIcon({className:'', html:svg, iconSize:[32,44], iconAnchor:[16,44], popupAnchor:[0,-38]});
  }
  ...
  CAFES.forEach(function(c){
    ...
    var html = "<div class='pop'><div class='pn'>"+esc(c.name)+"</div>"
    ...
    var mk = L.marker([c.lat, c.lon], {icon:pinIcon('#ff6fa5', false)}).addTo(map);
    mk.bindPopup(html, {maxWidth:260});
    mk.bindTooltip(esc(c.name), {className:'tt', direction:'top', offset:[0,-30]});
    pts.push([c.lat, c.lon]);
  });
  var am = L.marker([ACAD.lat, ACAD.lon], {icon:acadIcon(), zIndexOffset:1000}).addTo(map);
  am.bindPopup("<div class='pop acad'><div class='pn'>🏫 "+esc(ACAD.popup)+"</div>"
    + "<div class='sub'>"+esc(ACAD.sub)+"</div></div>", {maxWidth:240});
  am.bindTooltip(esc(ACAD.tip), {className:'tt tt-acad', direction:'top', offset:[0,-38]});
  ...
  if (map.getZoom() > 17) map.setZoom(17);
    ...
    st.markdown("<div class='map-hint'>📍 핀을 클릭하면 카페 정보와 네이버 지도 버튼이 떠요 · "
                "🖱️ 스크롤로 확대/축소 · 🏫 노란 핀은 학원</div>", unsafe_allow_html=True)
