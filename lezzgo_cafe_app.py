...
st.markdown(
    ...
    <style>
      ...
      iframe[title="streamlit_folium.st_folium"]{ border:2px solid var(--line) !important;
        border-radius:12px !important; overflow:hidden; box-shadow:0 8px 24px rgba(0,0,0,.45);}
    </style>
    """,
...
# ──────────────────────────────────────────────────────────────
# 카페 전체 보기 — 지도 (첨부 folium 코드 결합 / 좌표 컬럼 사용)
# ──────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def load_map_cafes():
    """i3_cafelist_최종.xlsx의 경도/위도를 읽어 마커용 리스트로. 겹치는 좌표는 살짝 분산."""
    import pandas as pd, math
    df = pd.read_excel("i3_cafelist_최종.xlsx", sheet_name="카페매칭")
    ...
    for _, r in df.iterrows():
        try:
            ...
            continue
        rows.append({
            "name": val(r.get("카페 이름")), "group": val(r.get("그룹명")),
            "review": val(r.get("한줄평")).replace("*", ""),
            "dist_min": val(r.get("거리(분)")), "dist_m": val(r.get("거리(m)")),
            "price": val(r.get("아메리카노 가격")), "hours": val(r.get("영업시간")),
            "link": val(r.get("네이버 지도 링크")), "lat": lat, "lon": lon,
        })

    # 동일 좌표(같은 건물) 마커가 겹치지 않게 소폭 회전 분산
    LAT_DIST = 111000.0
    seen = {}
    for c in rows:
...
def render_map():
    ...
    try:
        ...
        st.markdown(
            "<div class='map-warn'>지도 데이터(<code>i3_cafelist_최종.xlsx</code>)를<br>"
            "이 앱과 같은 폴더에 두고 다시 실행해 주세요.</div>", unsafe_allow_html=True)
        return
    except Exception as e:
        st.markdown(f"<div class='map-warn'>지도 데이터를 불러오지 못했어요<br>{e}</div>",
                    unsafe_allow_html=True)
        return
    if not cafes:
    ...
    view = pdk.ViewState(latitude=sum(lats) / len(lats),
                         longitude=sum(lons) / len(lons), zoom=15.4)

    cafe_layer = pdk.Layer(
        "ScatterplotLayer", data=cafe_rows, get_position="[lon, lat]",
        get_fill_color=[255, 111, 165, 230], get_radius=20,
        radius_min_pixels=6, radius_max_pixels=15, stroked=True,
        get_line_color=[255, 255, 255, 170], line_width_min_pixels=1, pickable=True)
    start_layer = pdk.Layer(
        "ScatterplotLayer", data=[start], get_position="[lon, lat]",
        get_fill_color=[255, 80, 95, 240], get_radius=28,
        radius_min_pixels=8, radius_max_pixels=20, stroked=True,
    ...
    st.markdown("<div class='map-hint'>🖱️ 마커에 올리면 카페 정보가 보여요</div>",
                unsafe_allow_html=True)

    # 카페 선택 → 네이버 지도 열기 (마커 클릭 대체)
