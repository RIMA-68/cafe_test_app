...
if page == "intro":
    ...
    if len(cafes) > 1:
        dc = st.columns([1, 2, 1])
        if dc[1].button("🎲 다른 카페 추천받기", use_container_width=True):
            others = [c for c in cafes if c["name"] != cafe["name"]] or cafes
            st.session_state.result_pick = {"group": no, "cafe": random.choice(others)}
            st.rerun()
    st.write("")
