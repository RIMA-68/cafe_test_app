...
<script>
  ...
  function pinIcon(){
    var svg = "<div class='pin'><svg width='30' height='42' viewBox='0 0 30 42'>"
      ...
      +   "fill='#ff6fa5' stroke='#3a2233' stroke-width='2.2' stroke-linejoin='round'/>"
      + "<circle cx='15' cy='14' r='5.2' fill='#fff6fb'/>"
      + "<circle cx='15' cy='14' r='2.2' fill='#ff6fa5'/>"
      + "</svg></div>";
  ...
  CAFES.forEach(function(c){
    ...
    var mk = L.marker([c.lat, c.lon], {icon:pinIcon()}).addTo(map);
    mk.bindPopup(html, {maxWidth:260});
    mk.bindTooltip(esc(c.name), {className:'tt', direction:'top', offset:[0,-36]});
