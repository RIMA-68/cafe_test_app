...
def load_map_cafes():
    """내장된 실제 좌표 데이터를 그대로 반환 (중복 카페 제거 완료, 좌표 보정 없음)."""
    ...
    for (lat, lon), mem in groups.items():
        ...
        for i, c in enumerate(mem):
            ...
            markers.append({
                ...
                "name": c["name"], "dist_min": c["dist_min"], "dist_m": c["dist_m"],
                "americano": price(c["americano"]), "decaf": price(c["decaf"]),
                "hours": c["hours"], "address": c["address"],
                "review": c["review"], "link": c["link"],
            })
