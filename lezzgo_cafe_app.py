...
st.markdown(
    ...
    <style>
      ...
      .q-title{ text-align:center; color:var(--ink); font-family:'Galmuri11','DungGeunMo',monospace; font-weight:700;
        font-size:clamp(21px,5.6vw,27px); line-height:1.6; margin:0 auto 48px; max-width:430px;
        text-shadow:0 2px 0 var(--pink-deep), 2px 2px 0 var(--pink-deep), 2px 3px 0 var(--black);}
...
def question(step, title, opt_a, opt_b, key, val_a, val_b, nxt, prev):
    ...
    if st.button(opt_a, key=f"{key}_a", type="primary"):
        st.session_state.ans[key] = val_a; go(nxt)
    if st.button(opt_b, key=f"{key}_b", type="primary"):
        st.session_state.ans[key] = val_b; go(nxt)
    st.markdown("<div style='height:20px'></div>", unsafe_allow_html=True)
    if st.button("← 이전으로", key=f"{key}_back", type="secondary"):
        go(prev)
...
if page == "intro":
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.write("")
    if st.button("🔄 다시 테스트하기", type="primary"):
