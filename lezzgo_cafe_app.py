...
st.markdown(
    ...
    <style>
      ...
      .cafe{ display:block; text-decoration:none; background:var(--screen);
      ...
      a.cafe{ cursor:pointer;}
      a.cafe:hover{ border-color:var(--pink); box-shadow:0 5px 16px rgba(255,111,165,.22);
      ...
      .cafe-map{ font-family:'Galmuri9',monospace; font-size:11px; color:var(--pink);
        margin-top:10px; text-align:right; opacity:.85;}
      a.cafe:hover .cafe-map{ opacity:1;}
    </style>
    """,
...
def cafe_card(cf):
    ...
    link = cf.get("link") or ""
    tag, href = ("a", f" href='{link}' target='_blank' rel='noopener'") if link else ("div", "")
    return (
        f"<{tag} class='cafe'{href}>"
        ...
        f"<div class='cafe-map'>📍 네이버 지도에서 보기 →</div>"
        f"</{tag}>"
    )
