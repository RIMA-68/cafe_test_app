...
def load_map_cafes():
    """내장 좌표 데이터를 반환. 같은 건물 등 겹치는 좌표는 살짝 회전 분산."""
    import math
    rows = [dict(c) for c in MAP_CAFES]
    LAT_DIST = 111000.0
    seen = {}
    for c in rows:
        ...
        n = seen.get(key, 0)
        if n > 0:
            ang = math.radians(n * 60)
            lon_dist = 111000.0 * math.cos(math.radians(c["lat"]))
            c["lat"] += (6.0 * math.sin(ang)) / LAT_DIST
            c["lon"] += (6.0 * math.cos(ang)) / lon_dist
        seen[key] = n + 1
    return rows
...
def render_map():
    ...
    ref_lat, ref_lon = 37.497942, 127.027621  # 강남역 기준점
    pts = [(c["lat"], c["lon"]) for c in cafes] + [(ref_lat, ref_lon)]
    lat0 = sum(p[0] for p in pts) / len(pts)
    lon0 = sum(p[1] for p in pts) / len(pts)
    coslat = math.cos(math.radians(lat0))

    def meters(lat, lon):
        return ((lon - lon0) * 111000.0 * coslat, (lat - lat0) * 111000.0)

    mxy = [meters(la, lo) for la, lo in pts]
    minx, maxx = min(p[0] for p in mxy), max(p[0] for p in mxy)
    miny, maxy = min(p[1] for p in mxy), max(p[1] for p in mxy)
    W, H, PAD = 700, 440, 54
    spanx, spany = (maxx - minx) or 1.0, (maxy - miny) or 1.0
    scale = min((W - 2 * PAD) / spanx, (H - 2 * PAD) / spany)
    offx, offy = (W - spanx * scale) / 2, (H - spany * scale) / 2

    def to_svg(lat, lon):
        x, y = meters(lat, lon)
        return offx + (x - minx) * scale, H - (offy + (y - miny) * scale)  # 북쪽이 위
    ...
    markers = []
    for c in cafes:
        sx, sy = to_svg(c["lat"], c["lon"])
        title = esc(f"{c['name']} · {c['dist_min']}분 · 아메리카노 {c['price']}원")
        dot = f"<circle class='mk' cx='{sx:.1f}' cy='{sy:.1f}' r='7'/><title>{title}</title>"
        link = str(c["link"])
        markers.append(f"<a href='{esc(link)}' target='_blank' rel='noopener'>{dot}</a>"
                       if link.startswith("http") else dot)
    markers_svg = "".join(markers)

    rsx, rsy = to_svg(ref_lat, ref_lon)
    ref_svg = (f"<g class='ref'><circle cx='{rsx:.1f}' cy='{rsy:.1f}' r='9'/>"
               f"<text x='{rsx:.1f}' y='{rsy - 15:.1f}'>강남역</text>"
               f"<title>강남역 기준점</title></g>")

    grid = "".join(f"<line x1='{i}' y1='0' x2='{i}' y2='{H}'/>" for i in range(0, W + 1, 40))
    grid += "".join(f"<line x1='0' y1='{j}' x2='{W}' y2='{j}'/>" for j in range(0, H + 1, 40))

    html_map = f"""<!DOCTYPE html><html><head><meta charset='utf-8'><style>
      @font-face{{ font-family:'Galmuri9'; font-display:swap;
        src:url('https://cdn.jsdelivr.net/gh/quiple/galmuri@latest/dist/Galmuri9.woff2') format('woff2'); }}
      *{{ margin:0; box-sizing:border-box; }}
      body{{ background:transparent; }}
      .wrap{{ border:2px solid rgba(255,111,165,.22); border-radius:12px; overflow:hidden;
              box-shadow:0 8px 24px rgba(0,0,0,.45); }}
      svg{{ display:block; width:100%; height:auto; background:
            radial-gradient(120% 120% at 50% 40%, #1f1226 0%, #150b1b 75%); }}
      .grid line{{ stroke:rgba(255,111,165,.10); stroke-width:1; }}
      .mk{{ fill:#ff6fa5; stroke:#fff; stroke-width:1.5; cursor:pointer;
            transition:r .1s ease, fill .1s ease; }}
      .mk:hover{{ fill:#ffd1e6; r:12; stroke-width:2.5; }}
      a{{ cursor:pointer; }}
      .ref circle{{ fill:#ff5a5f; stroke:#fff; stroke-width:2; }}
      .ref text{{ fill:#f6efe2; font-family:'Galmuri9',monospace; font-size:12px; text-anchor:middle;
                  paint-order:stroke; stroke:#0c0810; stroke-width:3px; }}
      .legend{{ fill:#efe7da; font-family:'Galmuri9',monospace; font-size:12px; }}
    </style></head><body>
      <div class='wrap'><svg viewBox='0 0 {W} {H}' xmlns='http://www.w3.org/2000/svg'>
        <g class='grid'>{grid}</g>
        {markers_svg}
        {ref_svg}
        <g transform='translate(18,{H - 18})'>
          <circle cx='6' cy='-4' r='6' fill='#ff6fa5' stroke='#fff' stroke-width='1.5'/>
          <text class='legend' x='18' y='0'>추천 카페</text>
          <circle cx='118' cy='-4' r='6' fill='#ff5a5f' stroke='#fff' stroke-width='1.5'/>
          <text class='legend' x='130' y='0'>강남역 기준</text>
        </g>
      </svg></div>
    </body></html>"""
    components.html(html_map, height=470, scrolling=False)
    st.markdown("<div class='map-hint'>🖱️ 마커에 올리면 카페 정보 · 클릭하면 네이버 지도로 이동</div>",
                unsafe_allow_html=True)
