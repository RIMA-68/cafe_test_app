...
def render_map():
    try:
        ...
        st.markdown(
            "<div class='map-warn'>지도 모듈(pydeck)을 불러오지 못했어요<br>"
            f"<code>{type(e).__name__}: {e}</code><br>"
            "<code>python -m pip install streamlit</code> 재설치로 해결돼요.</div>",
            unsafe_allow_html=True)
        return
    ...
    start = {"lat": 37.497942, "lon": 127.027621,
             "name": "📍 강남역 기준점", "review": "여기서 출발!", "dist_min": "-", "price": "-"}
    cafe_rows = [{"lat": c["lat"], "lon": c["lon"], "name": c["name"],
                  "review": c["review"], "dist_min": c["dist_min"], "price": c["price"]}
                 for c in cafes]

    lats = [r["lat"] for r in cafe_rows] + [start["lat"]]
    lons = [r["lon"] for r in cafe_rows] + [start["lon"]]
    view = pdk.ViewState(latitude=sum(lats) / len(lats),
                         longitude=sum(lons) / len(lons), zoom=15.4, pitch=0)

    cafe_layer = pdk.Layer(
        "ScatterplotLayer", data=cafe_rows, get_position="[lon, lat]",
        get_fill_color=[255, 111, 165, 230], get_radius=20,
        radius_min_pixels=6, radius_max_pixels=16, stroked=True,
        get_line_color=[255, 255, 255, 180], line_width_min_pixels=1.5, pickable=True)
    start_layer = pdk.Layer(
        "ScatterplotLayer", data=[start], get_position="[lon, lat]",
        get_fill_color=[255, 80, 95, 245], get_radius=30,
        radius_min_pixels=9, radius_max_pixels=22, stroked=True,
        get_line_color=[255, 255, 255, 230], line_width_min_pixels=2, pickable=True)

    tooltip = {
        "html": "<b>{name}</b><br/>{review}<br/>거리 {dist_min}분 · 아메리카노 {price}원",
        "style": {"backgroundColor": "#170d1d", "color": "#f6efe2",
                  "fontFamily": "monospace", "fontSize": "12px",
                  "border": "1px solid #ff6fa5", "borderRadius": "6px"},
    }
    deck = pdk.Deck(
        layers=[cafe_layer, start_layer], initial_view_state=view,
        map_style="https://basemaps.cartocdn.com/gl/dark-matter-gl-style/style.json",
        tooltip=tooltip)
    st.pydeck_chart(deck, use_container_width=True)
    st.markdown("<div class='map-hint'>🖱️ 스크롤·핀치로 확대/축소 · 드래그로 이동 · 마커에 올리면 카페 정보</div>",
                unsafe_allow_html=True)

    names = [c["name"] for c in cafes]
    sel = st.selectbox("카페를 골라 네이버 지도로 이동", names, key="map_sel")
    chosen = next((c for c in cafes if c["name"] == sel), None)
    if chosen and str(chosen["link"]).startswith("http"):
        st.link_button("📍 네이버 지도에서 보기 →", chosen["link"], use_container_width=True)
