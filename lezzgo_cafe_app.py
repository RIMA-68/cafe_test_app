...
def render_map():
    try:
        ...
        st.markdown(
            "<div class='map-warn'>지도 기능을 쓰려면 설치가 필요해요<br>"
    ...
    gangnam = (37.497942, 127.027621)  # 강남역 기준점
    positions = [list(gangnam)] + [[c["lat"], c["lon"]] for c in cafes]

    fmap = folium.Map(location=gangnam, zoom_start=16, tiles="CartoDB dark_matter")

    folium.Marker(
        location=list(gangnam),
        popup=folium.Popup("<b style='font-family:monospace'>📍 강남역 기준점</b>", max_width=160),
        tooltip="강남역",
    ...
    for c in cafes:
        ...
        link_html = (f"<a href='{link}' target='_blank' "
                     f"style='color:#ff6fa5;font-weight:bold;text-decoration:none;'>네이버 지도 열기 →</a>"
                     if link.startswith("http") else c["link"])
        popup_html = f"""
        <div style='font-family:monospace; width:230px; font-size:12px; line-height:1.55; color:#1a1320;'>
          <div style='background:#ffe3ef; border-left:4px solid #ff6fa5; border-radius:5px;
                      padding:6px 8px; margin-bottom:7px;'>💬 {c['review']}</div>
          <b>· {c['name']}</b><br>
          <b>거리</b> {c['dist_min']}분 ({c['dist_m']}m)<br>
          <b>아메리카노</b> {c['price']}원<br>
          <b>영업</b> {c['hours']}<br>
    ...
    fmap.fit_bounds(positions, padding=(30, 30))
