...
실행:  streamlit run lezzgo_cafe_app_with_map.py
필요:  pip install streamlit pandas openpyxl folium
DB:    결과 유형 데이터는 본 파일에 내장 + 전체 지도는 i3_cafelist.xlsx 기반
"""
...
try:
    import pandas as pd
except ImportError:  # 배포 환경에 pandas가 없을 때 앱 전체가 죽지 않게 처리
    pd = None

try:
    import folium
except ImportError:  # 배포 환경에 folium이 없을 때 안내 메시지 표시
    folium = None
...
CAFES  = {int(k): v for k, v in json.loads(r"""{
  ...
  "8": [
    ...
    },
    {
      ...
      "hours": "매일 00:00 - 24:00",
      "link": "https://map.naver.com/p/entry/place/1097569467?c=15.00,0,0,0,dh&placePath=/home?from=map&fromPanelNum=1&additionalHeight=76&timestamp=202606111339&locale=ko&svcName=map_pcv5",
      "review": "분위기 깡패 😳, 테이크아웃하려다 눌러 앉을수도"
    },
    {
      "name": "Cafe BONO (보노)",
      "dist": "3분",
...
st.markdown(
    ...
    <style>
      ...
      .map-hero{ text-align:center; margin:6px auto 18px; }
      .map-kicker{ font-family:'Galmuri9',monospace; color:var(--pink-soft); letter-spacing:3px; font-size:13px; margin-bottom:14px; }
      .map-title{ display:inline-block; color:var(--ink); font-family:'Galmuri11',monospace;
        font-size:clamp(28px,7vw,42px); line-height:1.15; padding:13px 23px 16px;
        background:var(--pink); border-radius:22px 18px 24px 18px; box-shadow:0 0 0 4px var(--pink-deep), 3px 5px 0 var(--black);
        text-shadow:2px 3px 0 var(--black); }
      .map-desc{ color:#efe7da; font-family:'Galmuri11',monospace; font-size:13px; line-height:1.8; max-width:430px; margin:22px auto 20px; text-align:center; }
      .map-panel{ background:rgba(23,13,29,.78); border:1px solid #2a1d33; border-left:4px solid var(--pink);
        border-radius:10px; padding:12px; margin:10px 0 18px; box-shadow:0 14px 36px rgba(0,0,0,.25); }
      .map-meta{ display:flex; justify-content:center; gap:8px; flex-wrap:wrap; margin:0 auto 14px; }
      .map-meta .b{ font-family:'Galmuri9',monospace; font-size:12px; color:var(--ink);
        background:rgba(255,111,165,.12); border:1px solid var(--line); border-radius:5px; padding:7px 11px; }
      .map-note{ font-family:'Galmuri9',monospace; font-size:11px; color:#9a8ba6; text-align:center; line-height:1.6; margin-top:10px; }
...
CAFE_LIST_PATH = APP_DIR / "i3_cafelist.xlsx"


def _clean_value(val, default="데이터 없음"):
    """엑셀 결측치/빈칸을 지도 팝업용 문자열로 정리합니다."""
    if val is None:
        return default
...
def _parse_coord(coord_text):
    """엑셀 '좌표' 컬럼의 '경도, 위도' 값을 folium용 '위도, 경도'로 변환합니다."""
    txt = _clean_value(coord_text, "")
    if not txt:
        return None, None
    try:
        lon, lat = [float(x.strip()) for x in txt.split(",")[:2]]
        return lat, lon
    except Exception:
        return None, None
...
@st.cache_data
def load_cafe_map_data():
    """i3_cafelist.xlsx의 카페매칭 시트를 읽어 중복 카페를 정리하고 좌표를 붙입니다."""
    if pd is None:
        raise ImportError("pandas 패키지가 필요합니다. requirements.txt에 pandas, openpyxl을 추가해 주세요.")
    if not CAFE_LIST_PATH.exists():
        raise FileNotFoundError(f"{CAFE_LIST_PATH.name} 파일을 앱 파일과 같은 폴더에 넣어 주세요.")

    raw = pd.read_excel(CAFE_LIST_PATH, sheet_name="카페매칭")
    required = ["그룹명", "카페 이름", "거리(분)", "거리(m)", "아메리카노 가격", "디카페인 가격", "영업시간", "네이버 지도 링크", "주소", "좌표", "한줄평"]
    missing = [col for col in required if col not in raw.columns]
    if missing:
        raise ValueError(f"엑셀 파일에 필요한 컬럼이 없습니다: {', '.join(missing)}")

    # 같은 카페가 여러 유형 그룹에 반복되어 있어, 전체 지도에서는 한 번만 보이도록 묶습니다.
    grouped = (
        raw.groupby(["카페 이름", "주소", "좌표"], dropna=False, as_index=False)
        .agg({
            "그룹명": lambda s: " / ".join(dict.fromkeys(_clean_value(x, "") for x in s if _clean_value(x, ""))),
            "거리(분)": "first",
            "거리(m)": "first",
            "아메리카노 가격": "first",
            "디카페인 가격": "first",
    ...
    coords = grouped["좌표"].apply(_parse_coord)
    grouped["위도"] = coords.apply(lambda x: x[0])
    grouped["경도"] = coords.apply(lambda x: x[1])
    grouped = grouped.dropna(subset=["위도", "경도"]).reset_index(drop=True)
    return grouped


def _offset_duplicate_markers(cafedata):
    """동일/인접 좌표 마커가 겹치지 않도록 첨부 지도 코드의 분산 배치 방식을 적용합니다."""
    gangnam_lat, gangnam_lon = 37.497942, 127.027621
    lat_dist = 111000.0
    lon_dist = 111000.0 * math.cos(math.radians(gangnam_lat))

    adjusted = []
    history = {}
    subway_count = 0

    for _, row in cafedata.iterrows():
        name = _clean_value(row.get("카페 이름"), "")
        lat = float(row["위도"])
        lon = float(row["경도"])

        if "강남역사" in name or "지하도" in name:
            radius = 10.0
            angle = subway_count * 45
            lat = gangnam_lat + (radius * math.sin(math.radians(angle))) / lat_dist
            lon = gangnam_lon + (radius * math.cos(math.radians(angle))) / lon_dist
            subway_count += 1
        else:
            ...
            if key in history:
                ...
                radius = 5.0
                angle = count * 60
                lat += (radius * math.sin(math.radians(angle))) / lat_dist
                lon += (radius * math.cos(math.radians(angle))) / lon_dist
            else:
                history[key] = 0
...
def _price_text(val):
    txt = _clean_value(val)
    if txt in ["-", "데이터 없음"]:
        return "데이터 없음"
    try:
        return f"{int(float(txt)):,}원"
    except Exception:
        return txt


def build_all_cafe_map(cafedata):
    """Folium 지도를 생성합니다. streamlit_folium 없이 components.html로 렌더링합니다."""
    if folium is None:
        raise ImportError("folium 패키지가 필요합니다. requirements.txt에 folium을 추가해 주세요.")

    start_lat, start_lon = 37.498085, 127.027978  # 서울 강남구 테헤란로1길 16 인근
    adjusted = _offset_duplicate_markers(cafedata)
    all_positions = [[start_lat, start_lon]] + [[lat, lon] for lat, lon in adjusted]
    center_lat = sum(pos[0] for pos in all_positions) / len(all_positions)
    center_lon = sum(pos[1] for pos in all_positions) / len(all_positions)

    m = folium.Map(location=[center_lat, center_lon], zoom_start=16, tiles="CartoDB positron")

    start_popup_html = """
    <div style='text-align:center; font-weight:700; font-family:sans-serif; padding:6px; line-height:1.5;'>
      😎😎😎<br>천재 마케터들의 소굴<br>😎😎😎
    </div>
    """
    folium.Marker(
        location=[start_lat, start_lon],
        popup=folium.Popup(start_popup_html, max_width=210),
        tooltip=folium.Tooltip("천마들", permanent=True),
        icon=folium.Icon(color="red", icon="info-sign"),
    ).add_to(m)

    for (_, row), (lat, lon) in zip(cafedata.iterrows(), adjusted):
        review = escape(_clean_value(row.get("한줄평")))
        group = escape(_clean_value(row.get("그룹명")))
        cafe_name = escape(_clean_value(row.get("카페 이름")))
        dist_min = escape(_clean_value(row.get("거리(분)")))
        dist_m = escape(_clean_value(row.get("거리(m)")))
        americano = escape(_price_text(row.get("아메리카노 가격")))
        decaf = escape(_price_text(row.get("디카페인 가격")))
        hours = escape(_clean_value(row.get("영업시간")))
        naver_link = _clean_value(row.get("네이버 지도 링크"), "")

        if naver_link.startswith("http"):
            link_html = f"<a href='{escape(naver_link, quote=True)}' target='_blank' style='color:#03C75A; font-weight:700; text-decoration:none;'>네이버 지도 열기</a>"
        else:
            link_html = escape(naver_link or "데이터 없음")

        popup_html = f"""
        <div style='font-family:sans-serif; width:250px; font-size:13px; line-height:1.55;'>
          <div style='background:#fff1f7; padding:7px; border-radius:6px; margin-bottom:8px; border-left:4px solid #ff6fa5;'>
            <strong>💬 한줄평</strong><br><span style='color:#333;'>{review}</span>
          </div>
          <b>• 유형:</b> {group}<br>
          <b>• 카페명:</b> {cafe_name}<br>
          <b>• 거리:</b> {dist_min} / {dist_m}m<br>
          <b>• 아메리카노:</b> {americano}<br>
          <b>• 디카페인:</b> {decaf}<br>
          <b>• 영업시간:</b> {hours}<br>
          <b>• 바로가기:</b> {link_html}
        </div>
        ...
        folium.Marker(
            ...
            tooltip=folium.Tooltip(_clean_value(row.get("카페 이름"))),
            icon=folium.Icon(color="pink", icon="coffee", prefix="fa"),
        ).add_to(m)
...
def render_all_cafe_page():
    st.markdown(
        "<div class='map-hero'>"
        "<div class='map-kicker'>★ ALL CAFE MAP ★</div>"
        "<div class='map-title'>카페 전체 보기</div>"
        "<div class='map-desc'>첨부한 <b>i3_cafelist.xlsx</b>의 카페 목록을 기반으로<br>학원 근처 카페를 한 번에 확인할 수 있어요.</div>"
        "</div>",
        unsafe_allow_html=True,
    )

    try:
        cafedata = load_cafe_map_data()
        st.markdown(
            f"<div class='map-meta'><span class='b'>☕ 전체 {len(cafedata)}곳</span>"
            "<span class='b'>📍 마커 클릭 시 상세정보</span>"
            "<span class='b'>💗 핑크 마커 = 카페</span></div>",
            unsafe_allow_html=True,
        )
        st.markdown("<div class='map-panel'>", unsafe_allow_html=True)
        cafe_map = build_all_cafe_map(cafedata)
        components.html(cafe_map.get_root().render(), height=560, scrolling=False)
        st.markdown("<div class='map-note'>좌표가 같은 카페는 마커가 겹치지 않도록 살짝 분산 배치했어요.</div></div>", unsafe_allow_html=True)
    except Exception as e:
        st.error(f"카페 전체 지도를 불러오지 못했어요: {e}")
        st.info("배포 시 app.py와 같은 폴더에 i3_cafelist.xlsx를 함께 올리고, requirements.txt에 pandas, openpyxl, folium을 추가해 주세요.")

    cc1, cc2 = st.columns(2)
    with cc1:
        if st.button("← 결과로 돌아가기", key="back_result", type="secondary", use_container_width=True):
            go("result" if st.session_state.ans else "intro")
    with cc2:
        if st.button("🔄 다시 테스트하기", key="map_restart", type="primary", use_container_width=True):
...
if page == "intro":
    ...
    with rc1:
        if st.button("🔄 다시 테스트하기", key="restart_from_result", type="primary", use_container_width=True):
            st.session_state.ans = {}
            go("intro")
    with rc2:
        if st.button("🗺️ 카페 전체 보기", key="all_cafe_map", type="primary", use_container_width=True):
            go("all_cafes")

elif page == "all_cafes":
    render_all_cafe_page()
